...
mkinitramfs 6.10.5
cp initrd.img-6.10.5 /boot

file_path="/boot/grub/grub.cfg"
# The string to search for
search_string="linux  /vmlinuz"

# The line to add after the matched line
line_to_add="initrd /initrd.img-6.10.5"

# Use sed to insert the line after the matching line
# sed -i "/$search_string/a $line_to_add" "$file_path"

# Use a temporary file and replace the original file
sed '/linux  \/vmlinuz/a initrd \/initrd.img-6.10.5' /boot/grub/grub.cfg >/boot/grub/grub.cfg.tmp && mv /boot/grub/grub.cfg.tmp /boot/grub/grub.cfg

echo "Line added after each occurrence of '$search_string'."
