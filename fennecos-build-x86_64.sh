...
function check_dependencies {
	...
	if [ ! -h /usr/bin/yacc -a "$(readlink -f /usr/bin/yacc)"="/usr/bin/bison.yacc" ]; then
		echo "WARNING: /usr/bin/yacc should be a link to bison, or a script that executes bison"
	fi

	# check that awk is a link to gawk
	if [ ! -h /usr/bin/awk -a "$(readlink -f /usr/bin/awk)"="/usr/bin/gawk" ]; then
...
function install_system_config {
	local FILENAME=$1
...
function install_distro_config {
	local FILENAME=$1
	local FULLPATH="$LFS/$(basename $FILENAME | sed 's/__/\//g')"
	mkdir -p $(dirname $FULLPATH)
...
function download_packages {
	if [ -n "$1" ]; then
		...
		local PACKAGE_DIR=$1/$ARCH/packages
		# local PACKAGE_LIST=$1/x86_64/packages.sh
		local PACKAGE_LIST=$1/$ARCH/packages.sh
	fi
	...
	for url in $PACKAGE_URLS; do
		...
		if ! echo $ALREADY_DOWNLOADED | grep $(basename $url) >/dev/null; then
...
function unmount_image {
	...
	if [ -n "$ATTACHED_LOOP" ]; then
		sudo losetup -d $(echo "$ATTACHED_LOOP" | cut -d" " -f1)
	fi
...
function build_package {
	...
	local SCRIPT_PATH=$([ $PHASE -eq 5 ] && echo $EXTENSION/scripts/${NAME}.sh || echo ./$ARCH/phase${PHASE}/${NAME}.sh)
	...
	if [ "$NAME_OVERRIDE" == "_" ]; then
		...
		if [ -z "${!PKG_NAME}" ] || [[ "${!PKG_NAME}" == "" ]]; then
			...
			*.tar)
				...
				;;
			*.git)
				local EXTRACTCMD="git clone ${FILE} $NAME"
				;;
			*)
...
function build_phase {
	...
	if [ -n "$STARTPHASE" ]; then
		if [ $PHASE -lt $STARTPHASE ] || { $FOUNDSTARTPHASE && $ONEOFF; }; then
	...
	local PHASE_DIR=./$ARCH/phase$PHASE
	...
	for ((i = 0; i < $PKG_COUNT; i++)); do
		...
		if $FOUNDSTARTPKG && $ONEOFF; then
			...
			return 0
		elif [ -n "$STARTPKG" ] && ! $FOUNDSTARTPKG; then
			# if start package is defined, skip until found
			if [ "$STARTPKG" == "$(echo $pkg | cut -d" " -f1)" ]; then
				FOUNDSTARTPKG=true
	...
	if [ -n "$STARTPKG" -a "$STARTPHASE" == "$PHASE" -a ! $FOUNDSTARTPKG ]; then
...
function build_extension {
	...
	elif [ ! -f "$EXTENSION/$ARCH/packages.sh" ]; then
	...
	source "$EXTENSION/$ARCH/packages.sh"
	...
	cp -f $EXTENSION/$ARCH/packages/* $LFS/var/cache/sources/

	# install system_config files if present
	if [ -d "$EXTENSION/system_config" ]; then
		for f in $EXTENSION/system_config/*; do
			install_system_config $f
		done
	fi

	# install distro_config files if present
	if [ -d "$EXTENSION/distro_configs" ]; then
		for f in $EXTENSION/distro_configs/*; do
			install_distro_config $f
		done
	fi
...
function install_image {
	...
	*)
	...
	if ! echo "$FDISK_INSTR" | fdisk $INSTALL_TGT |& { $VERBOSE && cat || cat >/dev/null; }; then
...
function clean_image {
	...
	# delete logs
	if [ -d $LOG_DIR ] && [ -n "$(ls $LOG_DIR)" ]; then
		rm $LOG_DIR/*
	fi
}
...
function main {
	...
	# phase 3 cleanup
	if $BUILDALL || [ "$STARTPHASE" -le "3" ]; then
...
OPCOUNT=0
for OP in BUILDALL CHECKDEPS DOWNLOAD INIT STARTPHASE MOUNT UNMOUNT INSTALL_TGT CLEAN CHROOT; do
	OP="${!OP}"
	if [ -n "$OP" -a "$OP" != "false" ]; then
		OPCOUNT=$((OPCOUNT + 1))
	fi

	if [ $OPCOUNT -gt 1 ]; then
...
if [ -n "$STARTPHASE" ]; then
	if ! [[ "$STARTPHASE" =~ ^[1-5]$ ]]; then
		...
		exit 1
	elif [ "$STARTPHASE" -eq 5 ] && [ -z "$EXTENSION" ]; then
...
if [ -n "$EXTENSION" ]; then
	if ! $BUILDALL && [ -z "$STARTPHASE" ]; then
		...
		exit 1
	elif $ONEOFF && [ "$STARTPHASE" -ne 5 ]; then
...
if $CHROOT; then
	...
	sudo ./fennecos-build-x86_64.sh -m
	...
	run_umount() {
		sudo ./fennecos-build-x86_64.sh -u
	}
